...
const LOG_FILE = path.join(__dirname, "procesar_convivencia_log.txt");
...
const OUTPUT_FILE = path.join(__dirname, "ley_convivencia.json");

const PDF_FILES = [
  { ruta: path.join(__dirname, "ley de convivencia.pdf"), ley: "Convivencia Escolar / 21.128" },
];
...
try {
  log("═══════════════════════════════════════════════════════════");
  log("  📜 Procesador de Leyes SLEP — Específico Convivencia");
  log("═══════════════════════════════════════════════════════════");

  const resultados = [];
  // Inicializamos un ID temporal
  let idGlobal = 1000;
  ...
  const salida = {
    metadata: {
      ...
      nota: "Texto completo de la ley de convivencia escolar.",
    },
